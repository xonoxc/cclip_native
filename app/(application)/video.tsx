	...
	const createVideo = useCallback(async () => {
		...
		try {
			...
			const uploadResp = await axios.post(
				`${process.env.EXPO_PUBLIC_CLOUDINARY_BASE_URL!}/${cloudinaryConfg.cloud_name}/video/upload`,
				formData,
				{
					headers: {
			...
			if (uploadResp.data) {
				const data = uploadResp.data as VideoUploadResult


				const response = await apiClient.post('/api/video-upload', {
					title,
					description,
						...
						<View className="space-y-6 pb-8">
							...
							{isUploading && uploadProgress && (
								<View className="my-3">
									...
									<Text className="text-white">{uploadProgress}% uploaded</Text>
								</View>
							)}
							<View className="space-y-3">
								...
								<View className="mt-5">
									<TouchableOpacity
										...
										className={`w-full py-4 rounded-xl ${isUploading
											? "bg-gray-400/80"
											: "bg-white active:opacity-80"
											}`}
									>
										<Text
											className={`text-center font-semibold text-base ${isUploading
												? "text-gray-600"
												: "text-black"
												}`}
										>
