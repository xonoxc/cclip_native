...
const useImageTransformation = ({ width, height, aspectRatio, publicID }: ITransformationArgs): ImageURLstringObj => {
	const baseURL = process.env.EXPO_PUBLIC_CLOUDINARY_FETCH_IMAGE_BASE_URL!
	if (!baseURL) {
		throw new Error("Please set EXPO_PUBLIC_CLOUDINARY_FETCH_IMAGE_BASE_URL in .env")
	}
	return {
